...
(function () {
  ...
  // ─── API Host Interceptor (for Android APK & Local Network) ───────────────
  const _originalFetch = window.fetch;
  window.fetch = function(input, init) {
    if (typeof input === "string" && input.startsWith("/api/")) {
      let serverBase = localStorage.getItem("lactoguard_server_url") || "";
      if (!serverBase && (window.location.protocol === "file:" || !window.location.host || window.location.origin === "null")) {
        serverBase = "http://12.10.3.159:5173";
      }
      if (serverBase) {
        input = serverBase.replace(/\/+$/, "") + input;
      }
    }
  ...
  function setupVoiceAssistantListeners() {
    ...
    function setListeningState(listening) {
      ...
      const s = document.getElementById("voiceStatusText");
      const e = document.getElementById("voiceEqualizer");
      if (b) {
        if (listening) {
          b.classList.add("listening");
          b.innerHTML = "🛑";
          if (s) s.innerText = "🎙️ Listening... Speak your question now (or click 🛑 when finished).";
          if (e) e.classList.add("active");
        } else {
          b.classList.remove("listening");
          b.innerHTML = "🎙️";
          if (s) s.innerText = "Click microphone to speak, search cattle above, or select a question below";
    ...
    function finishAndSubmit(textToSubmit) {
      ...
      stopAllListening();
      if (q) {
        const inputEl = document.getElementById("voiceInputField");
      ...
      try {
        ...
        recognition.onresult = (event) => {
          ...
          if (full) {
            ...
            if (statusEl) statusEl.innerText = `🎙️ Hearing: "${full}"...`;
        ...
        recognition.onerror = (event) => {
          ...
          if (event.error === "not-allowed" || event.error === "service-not-allowed") {
            const statusEl = document.getElementById("voiceStatusText");
            if (statusEl) statusEl.innerText = "Browser STT restricted. Using direct audio recording...";
          }
        };
    ...
    async function startClientMediaRecording() {
      if (!navigator.mediaDevices || !navigator.mediaDevices.getUserMedia) return;
      try {
        const stream = await navigator.mediaDevices.getUserMedia({ audio: true });
        clientAudioChunks = [];
        clientMediaRecorder = new MediaRecorder(stream);
        clientMediaRecorder.ondataavailable = (e) => {
          if (e.data && e.data.size > 0) clientAudioChunks.push(e.data);
        };
        clientMediaRecorder.onstop = async () => {
          // If browser speech already yielded query, skip upload
          if (currentCapturedText && currentCapturedText.length > 2) return;
          const audioBlob = new Blob(clientAudioChunks, { type: "audio/webm" });
          if (audioBlob.size < 800) return; // Too small / empty

          const statusEl = document.getElementById("voiceStatusText");
          if (statusEl) statusEl.innerText = "⚡ Transcribing audio with Groq Whisper AI...";
          ...
          reader.onloadend = async () => {
            ...
            try {
              ...
              if (data.success && data.text) {
                ...
                if (statusEl) statusEl.innerText = data.error || "No clear speech detected. Please speak or type.";
              }
            } catch (e) {
              if (statusEl) statusEl.innerText = "Transcription error. Please type your query below.";
            }
          };
        };
    ...
    function toggleMic() {
      if (isListening || state.voiceListening) {
        const inputEl = document.getElementById("voiceInputField");
        const textToSubmit = currentCapturedText || (inputEl ? inputEl.value.trim() : "");
        if (textToSubmit) {
      ...
      maxTimeoutTimer = setTimeout(() => {
        if (isListening) {
          if (currentCapturedText) {
            ...
            stopAllListening();
            const statusEl = document.getElementById("voiceStatusText");
            if (statusEl) statusEl.innerText = "Listening window ended. Click 🎙️ to try again or type below.";
          }
        }
      }, 20000);
      ...
      // Simultaneously start Client MediaRecorder for high-accuracy Groq Whisper fallback
      startClientMediaRecording();
    }
